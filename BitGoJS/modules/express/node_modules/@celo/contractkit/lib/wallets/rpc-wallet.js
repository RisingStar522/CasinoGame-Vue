"use strict";
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.RpcWallet = exports.RpcWalletErrors = void 0;
var address_1 = require("@celo/utils/lib/address");
var rpc_caller_1 = require("../utils/rpc-caller");
var remote_wallet_1 = require("./remote-wallet");
var rpc_signer_1 = require("./signers/rpc-signer");
var RpcWalletErrors;
(function (RpcWalletErrors) {
    RpcWalletErrors["FetchAccounts"] = "RpcWallet: failed to fetch accounts from server";
    RpcWalletErrors["AccountAlreadyExists"] = "RpcWallet: account already exists";
})(RpcWalletErrors = exports.RpcWalletErrors || (exports.RpcWalletErrors = {}));
/*
 *   WARNING: This class should only be used with well-permissioned providers (ie IPC)
 *   to avoid sensitive user 'privateKey' and 'passphrase' information being exposed
 */
var RpcWallet = /** @class */ (function (_super) {
    __extends(RpcWallet, _super);
    function RpcWallet(_provider) {
        var _this = _super.call(this) || this;
        _this._provider = _provider;
        _this.rpc = new rpc_caller_1.DefaultRpcCaller(_provider);
        return _this;
    }
    RpcWallet.prototype.loadAccountSigners = function () {
        return __awaiter(this, void 0, void 0, function () {
            var addressToSigner, resp, accounts;
            var _this = this;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        addressToSigner = new Map();
                        return [4 /*yield*/, this.rpc.call('eth_accounts', [])];
                    case 1:
                        resp = _a.sent();
                        if (resp.error) {
                            throw new Error(RpcWalletErrors.FetchAccounts);
                        }
                        accounts = resp.result;
                        accounts.forEach(function (account) {
                            addressToSigner.set(account, new rpc_signer_1.RpcSigner(_this.rpc, account));
                        });
                        return [2 /*return*/, addressToSigner];
                }
            });
        });
    };
    RpcWallet.prototype.addAccount = function (privateKey, passphrase) {
        return __awaiter(this, void 0, void 0, function () {
            var address, signer, resultantAddress;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        address = address_1.normalizeAddressWith0x(address_1.privateKeyToAddress(address_1.ensureLeading0x(privateKey)));
                        if (this.hasAccount(address)) {
                            throw new Error(RpcWalletErrors.AccountAlreadyExists);
                        }
                        signer = new rpc_signer_1.RpcSigner(this.rpc, address);
                        return [4 /*yield*/, signer.init(privateKey, passphrase)];
                    case 1:
                        resultantAddress = _a.sent();
                        this.addSigner(resultantAddress, signer);
                        return [2 /*return*/, resultantAddress];
                }
            });
        });
    };
    RpcWallet.prototype.unlockAccount = function (address, passphrase, duration) {
        return __awaiter(this, void 0, void 0, function () {
            var signer;
            return __generator(this, function (_a) {
                signer = this.getSigner(address);
                return [2 /*return*/, signer.unlock(passphrase, duration)];
            });
        });
    };
    RpcWallet.prototype.isAccountUnlocked = function (address) {
        var signer = this.getSigner(address);
        return signer.isUnlocked();
    };
    /**
     * Gets the signer based on the 'from' field in the tx body
     * @param txParams Transaction to sign
     * @dev overrides WalletBase.signTransaction
     */
    RpcWallet.prototype.signTransaction = function (txParams) {
        return __awaiter(this, void 0, void 0, function () {
            var fromAddress, signer;
            return __generator(this, function (_a) {
                fromAddress = txParams.from.toString();
                signer = this.getSigner(fromAddress);
                return [2 /*return*/, signer.signRawTransaction(txParams)];
            });
        });
    };
    return RpcWallet;
}(remote_wallet_1.RemoteWallet));
exports.RpcWallet = RpcWallet;
//# sourceMappingURL=rpc-wallet.js.map